...
import { Tooltip, TooltipTrigger } from '@c/ui';
...
const Tab = ({ href, label, isActive, disabled }: TabProps) => {
  ...
  const handleMouseEnter = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    const { left, top, right, bottom } = (target as HTMLElement).getBoundingClientRect();
  ...
  const tabContent = (
    <motion.div
      ...
      ${isActive ? "bg-dark-section--lighter text-white" : "bg-transparent text-muted hover:text-white"} ${disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
    ...
    <nav className="w-full pt-[12px] border-b border-seperator relative">
      <div className="max-w-full mx-auto px-4 relative">
        {showLeftArrow && (
          <button
            onClick={() => scroll('left')}
            className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-dark-section--lighter rounded-full p-1 z-10"
          >
            <ChevronLeft size={24} />
          </button>
        )}
