...
import { createWishlist, createWishlistItem, deleteWishlistItem, getWishlistsByUser } from '../server/server-actions/wishlist';
...
type WishlistStore = {
    ...
    addWishlistItem: (wishlistId: string, name: string, price: number, description: string) => Promise<void>;
...
export const useWishlistStore = create<WishlistStore>((set, get) => ({
    ...
    addWishlistItem: async (wishlistId: string, name: string, price: number, description: string): Promise<void> => {
        try {
            await createWishlistItem(wishlistId, name, price, description);
